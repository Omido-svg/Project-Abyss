...
public sealed class YujinMechanic : CombatMechanic
{
        ...
        YujinWeaponType weapon)
    {
        ...
        // 환형은 행동 선택 전에만 사용할 수 있다.
        BattleUIManager battleUi =
            battleContext?.battleManager?.BattleUIManager;

        if (battleUi != null &&
            battleUi.SelectedOwner == owner &&
            battleUi.InputMode != BattleInputMode.SelectOwner)
        {
            return false;
        }

        IReadOnlyList<ActionSlot> slots =
            battleContext?.battleManager?.ActionManager?.Slots;
    ...
    /// 실전 환형은 반드시 TrySwitchWeapon을 사용한다.
    /// </summary>
