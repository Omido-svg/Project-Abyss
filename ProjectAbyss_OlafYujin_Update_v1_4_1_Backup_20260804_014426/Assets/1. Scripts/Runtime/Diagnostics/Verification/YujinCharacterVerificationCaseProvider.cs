    ...
    ICharacterVerificationCaseProvider
{
            ...
            CharacterAuthoringBundle bundle)
    {
        ...
        yield return CharacterVerificationCaseDefinition.Create(
            ...
            CharacterVerificationCategory.Passive,
            CharacterVerificationExecutionMode.IsolatedRuntime,
            "유진의 행동 슬롯이 이미 생성된 뒤에는 환형이 거부되고 빛과 현재 무기가 유지되는지 검사합니다.");
            ...
            CharacterVerificationContext context)
    {
        ...
        bool canAfterSelection = false;
        bool switchedAfterSelection = false;
        bool canAfterClear = false;
        bool switchedAfterClear = false;
        int energyAfterBlocked = energyBefore;
        int energyAfterSuccess = energyBefore;
        YujinWeaponType weaponAfterBlocked =
            mechanic.CurrentWeapon;
        ...
        try
        {
            ...
            // Inactive 상태에서 Component를 추가해 Awake의 실전 초기화를 막는다.
        ...
        finally
        {
            ...
            if (manager != null)
            {
                // 검증 Stub의 OnDestroy가 실전 종료 경로를 실행하지 않도록 차단한다.
        ...
        bool valid =
            ...
            !canAfterSelection &&
            !switchedAfterSelection &&
            energyAfterBlocked == energyBefore &&
            weaponAfterBlocked == YujinWeaponType.Baeku &&
            canAfterClear &&
            switchedAfterClear &&
            energyAfterSuccess ==
                energyBefore -
        ...
        string detail =
            $"Before={canBeforeSelection}, " +
            $"Selected={canAfterSelection}/{switchedAfterSelection}, " +
            $"AfterClear={canAfterClear}/{switchedAfterClear}, " +
            $"Energy={energyBefore}→{energyAfterBlocked}→{energyAfterSuccess}, " +
            $"Weapon={weaponAfterBlocked}→{mechanic.CurrentWeapon}";

        return valid
            ? context.Pass(
                "행동 선택 전 사용 가능·선택 후 차단·슬롯 해제 후 빛 1 소비",
                detail)
            : context.Fail(
                "행동 선택 전 사용 가능·선택 후 차단·슬롯 해제 후 빛 1 소비",
                detail);
