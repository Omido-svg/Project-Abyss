...
public sealed class BattleParticipantButtonFactory : MonoBehaviour
{
    ...
    [Tooltip("좌측 플레이어 열과 우측 적 열 사이에 TargetArrowUI를 가로로 표시할 때 사용합니다.")]
        ...
        BattleParticipantSide side)
    {
        ...
        if (horizontalClashOverview)
        {
            // 전체 결투 현황에서는 적과 아군 모두 오른쪽 기준으로 정렬한다.
            // 버튼 수가 4개보다 적어도 양쪽 행의 중심축이 서로 어긋나지 않는다.
            grid.startCorner = GridLayoutGroup.Corner.UpperRight;
            grid.childAlignment = TextAnchor.MiddleRight;
        }
        else if (side == BattleParticipantSide.Enemy)
        {
            grid.startCorner =
    ...
    {
        // Scene unload / Play Mode 종료 중에는 자식 GameObject도 이미
        // 파괴 절차에 들어가므로 SetActive나 UI Refresh를 호출하면 안 됩니다.
        // 일반적인 재빌드 정리는 ClearGeneratedButtons()에서 수행합니다.
        generatedButtons.Clear();
    }
}