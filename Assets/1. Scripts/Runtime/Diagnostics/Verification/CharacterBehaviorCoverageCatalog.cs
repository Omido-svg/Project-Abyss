...
public static class CharacterBehaviorCoverageCatalog
{
        ...
        CharacterAuthoringBundle bundle)
    {
        ...
        switch (bundle?.Kind)
        {
            ...
            case CharacterAuthoringKind.Hifumi:
                required.Add(nameof(HifumiMechanic));
                break;
    ...
    private static void VerifyOlafDiceContracts(
        CharacterVerificationContext context,
        ICollection<string> failures,
        ICollection<string> observations)
    {
        (string SkillId, int BasePower, int RollCount)[] contracts =
        {
            (OlafSkillIds.EnduringSlash, 13, 3),
            (OlafSkillIds.OverheadSmash, 14, 2),
            (OlafSkillIds.WildHack, 16, 3),
            (OlafSkillIds.Standard, 16, 2),
            (OlafSkillIds.Rend, 12, 3)
        };

        bool allValid = true;

        foreach ((string skillId, int basePower, int rollCount) in contracts)
        {
            ...
            bool valid =
                definition != null &&
                definition.BasePower == basePower &&
                definition.Rolls != null &&
                definition.Rolls.Count == rollCount &&
                definition.Rolls.All(roll =>
                    roll != null &&
                    roll.RngSource == RollRngSource.Dice &&
                    roll.DiceMode == DicePowerMode.BasePlusRoll &&
                    roll.SafeMinPower == 1 &&
                    roll.SafeMaxPower == 8 &&
                    roll.GetDiceFinalMinPower(basePower) == basePower + 1 &&
                    roll.GetDiceFinalMaxPower(basePower) == basePower + 8);

            if (!valid)
            {
                allValid = false;
                failures.Add(
                    $"Olaf Dice 계약 불일치: {skillId}, " +
                    $"Base={definition?.BasePower}, " +
                    $"Rolls={definition?.Rolls?.Count ?? 0}");
            }
        }
        ...
        {
            observations.Add(
                "Olaf Dice: 공용 BasePlusRoll·스킬별 BasePower·공통 d8 PASS");
        }
    }
        ...
        ICollection<string> observations)
    {
        ...
        if (skillId == OlafSkillIds.Standard)
        {
            // 「표준」은 승리 시 혈상, 패배 시 광기를 각각 따로 준다.
            ClashExchangeResult losingExchange =
        ...
        {
            observations.Add(
                $"Olaf {skillId}: 최신 승패별 혈상·광기 규칙 PASS / " +
                $"Bleeding={bleeding}, Madness={mechanic.CurrentMadness}");
        }
    }
        ...
        ICollection<string> observations)
    {
        ...
        foreach (YujinWeaponType weapon in weapons)
        {
            if (mechanic.CurrentWeapon != weapon)
                mechanic.SetWeaponForVerification(weapon);
        ...
        ICollection<string> observations)
    {
        ...
        else
        {
            ...
            {
                observations.Add("올라프 터뜨리는 광기 광기+2·(광기×5+혈상) 피해 PASS");
            }
        }
        ...
        ICollection<string> observations)
    {
        ...
        {
            observations.Add("Elite 합 승리 +10/혈상1, 자가 약화 +15, 파괴 +25 PASS");
        }
    }
        ...
        ICollection<string> observations)
    {
        ...
        if (bleeding != 1)
            failures.Add($"피 냄새 합 승리 혈상 불일치: {bleeding}");
        else
            observations.Add("NormalEnemy 피 냄새 합 승리 혈상 1 PASS");
    }
}